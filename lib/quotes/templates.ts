...
export function renderQuoteSms(quote: Pick<Quote, "projectType">, url: string) {
  const parsed = new URL(url);
  if (parsed.protocol !== "https:" || parsed.username || parsed.password) throw new Error("quote_url_invalid");
  return `ASIR SOLAR: ${quote.projectType.replace(/[\u0000-\u001f\u007f]/g, " ").slice(0, 100)} projeniz için teklifiniz hazır. Teklifinizi inceleyin: ${url}`;
}
