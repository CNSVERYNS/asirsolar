...
import { finishQuoteDelivery, prepareQuoteSms, quoteJobAllowed } from "../quotes/notifications.ts";

export async function flushSmsOutbox(leadId?: string) {
  if (!smsEnabled()) return { configured: false, accepted: 0 };
  const db = getDatabase();
  await db.prepare("UPDATE sms_outbox SET status='unknown', retryable=false, claim_token=NULL, quote_token=NULL, error_code='interrupted' WHERE status='sending' AND claimed_at < ?").run(Date.now() - 5 * 60 * 1000);
  const candidates = await db.prepare(`SELECT id FROM sms_outbox WHERE status IN ('pending','failed') AND retryable=true AND attempts < 6 AND available_at <= ? AND (quote_id IS NULL OR ?=true) ${leadId ? "AND lead_id=?" : ""} ORDER BY sequence LIMIT 4`).all(Date.now(), quoteSendingEnabled(), ...(leadId ? [leadId] : [])) as { id: string }[];
  const results = await Promise.allSettled(candidates.map(async candidate => {
    const claim = randomUUID();
    const row = await db.prepare("UPDATE sms_outbox SET status='sending', attempts=attempts+1, claimed_at=?, claim_token=? WHERE id=? AND status IN ('pending','failed') AND retryable=true AND attempts < 6 AND available_at <= ? RETURNING lead_id, recipient, attempts, quote_id, quote_token").get(Date.now(), claim, candidate.id, Date.now()) as { lead_id: string; recipient: string; attempts: number; quote_id: string | null; quote_token: string | null } | undefined;
    if (!row) return false;
    let providerId: string | undefined;
    try {
      if (row.quote_id && !await quoteJobAllowed(row.quote_id, true)) {
      ...
      const retryable = !uncertain && failure.retryable && row.attempts < 6;
      await db.prepare("UPDATE sms_outbox SET status=?, retryable=?, provider_id=?, error_code=?, available_at=?, claim_token=NULL,quote_token=CASE WHEN ? THEN quote_token ELSE NULL END WHERE id=? AND claim_token=?").run(uncertain ? "unknown" : "failed", retryable, providerId ?? null, failure.code, retryAt(row.attempts), retryable, candidate.id, claim);
      return false;
    }
  }));
