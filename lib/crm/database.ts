...
export const projectSlugsMigrationPath = resolve("supabase/migrations/202609230002_project_slugs.sql");
export async function initializeDatabase() {
  if (!state.ready) state.ready = (async () => {
    if (process.env.DATABASE_URL) {
      ...
      await state.local.exec(await readFile(projectSlugsMigrationPath, "utf8"));
    }
...
function sqlForPostgres(sql: string) {
  ...
  return sql.replace(/\?/g, () => `$${++parameter}`)
    .replace(/\b(FROM|JOIN|INTO|UPDATE)\s+(users|sessions|leads|lead_events|email_outbox|sms_outbox|notification_worker|request_limits|projects|project_images)\b/gi, "$1 asir_crm.$2")
...
export async function migrateDatabase() {
  ...
  if (state.pool) await state.pool.query(await readFile(projectSlugsMigrationPath, "utf8"));
}
export async function migrateProjectSlugs() {
  await initializeDatabase();
  const sql = await readFile(projectSlugsMigrationPath, "utf8");
...
export async function migratePipelineModule() {
  await initializeDatabase();
  const sql = await readFile(pipelineMigrationPath, "utf8");
  if (state.pool) await state.pool.query(sql);
  else await state.local!.exec(sql);
}
export async function migrateQuoteModule() {
  await initializeDatabase();
  const sql = await readFile(quotesMigrationPath, "utf8");
  if (state.pool) await state.pool.query(sql);
  else await state.local!.exec(sql);
}
