...
export function AdminShell({ user, children }: { user: AdminUser; children: ReactNode }) {
  ...
  return <div className="crm-shell">
    <aside className="crm-sidebar"><Link href="/admin" className="crm-brand"><Mark size={47} /><span>ASIR SOLAR<small>İŞ TAKİBİ</small></span></Link><span className="crm-nav-label">ÇALIŞMA ALANI</span><nav aria-label="Panel navigasyonu"><Link href="/admin" data-active={pathname === "/admin" || pathname.startsWith("/admin/talepler")}><span aria-hidden="true">▤</span>Müşteri takibi</Link><Link href="/admin/crm" data-active={pathname.startsWith("/admin/crm")}><span aria-hidden="true">▥</span>CRM</Link><Link href="/admin/projeler" data-active={pathname.startsWith("/admin/projeler")}><span aria-hidden="true">▧</span>Projeler</Link><Link href="/admin/ayarlar" data-active={pathname.endsWith("ayarlar")}><span aria-hidden="true">⚙</span>Hesabım ve bağlantılar</Link></nav><div className="crm-sidebar__bottom"><Link href="/" target="_blank" rel="noreferrer">Web sitesini aç <span aria-hidden="true">↗</span></Link><div className="crm-person"><span className="crm-avatar">{user.name.split(" ").map(part => part[0]).slice(0, 2).join("")}</span><div><strong>{user.name}</strong><small>Asır Solar ekibi</small></div></div><button type="button" onClick={logout} disabled={busy}>{busy ? "Çıkış yapılıyor…" : "Güvenli çıkış"}</button>{error && <small role="alert">{error}</small>}</div></aside>
