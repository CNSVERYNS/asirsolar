...
export function LeadWorkspace({ initialDetail, users, quotes }: { initialDetail: LeadDetail; users: AdminUser[]; quotes: QuoteDetail[] }) {
  ...
  return <>
    ...
    <div className="crm-panel crm-quote-customer"><div><span>Müşteri</span><strong>{lead.name}</strong></div><div><span>Telefon</span><strong>{lead.phone || "—"}</strong></div><div><span>E-posta</span><strong>{lead.email || "—"}</strong></div><div><span>Proje</span><strong>{lead.projectType}</strong></div></div>
    <QuoteList lead={lead} initial={quotes} />
