...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // /kvkk, /gizlilik ve /cerezler bilinçli olarak
  ...
  const managedProjectRoutes = publishedProjects.map(project => ({ url: `${siteConfig.url}/projeler/${project.slug}`, lastModified: project.updatedAt, changeFrequency: "monthly" as const, priority: 0.6 }));
  ...
  const cityRoutes = [...new Set(localAreas.map(area => area.citySlug))].map(city => ({ url: `${siteConfig.url}/bolgeler/${city}`, lastModified: "2026-09-22", changeFrequency: "monthly" as const, priority: 0.6 }));
