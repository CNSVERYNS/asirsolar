...
try {
  ...
  assert.equal((await request('/admin/projeler')).status,307);
  assert.equal((await request('/api/admin/projeler')).status,401);
  assert.equal((await request('/api/admin/projeler',{method:'POST',body:{}})).status,401);
  assert.equal((await request('/api/admin/projeler/missing',{method:'PATCH',body:{}})).status,401);
  assert.equal((await request('/api/admin/projeler/missing',{method:'DELETE'})).status,401);
  assert.equal((await request('/api/admin/projeler/missing/gorseller',{method:'POST',body:{}})).status,401);
  ...
  const signedIn=await request('/api/admin/giris',{method:'POST',body:{username:adminAccounts[0].username,password}});
  ...
  assert.equal((await request('/admin/projeler',{auth})).status,200);
  const projectInput={name:'HTTP test project <script>alert(1)</script>',description:'Test project description',startDate:'2026-09-15',endDate:'',published:false};
  assert.equal((await request('/api/admin/projeler',{method:'POST',auth,origin:'https://attacker.invalid',body:projectInput})).status,403);
  ...
  const image=(await json(await upload(png),201)).image;
