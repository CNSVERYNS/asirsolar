...
const {renderNotificationEmail,renderTeamSms} = await import('../lib/crm/notification-templates.ts');
...
test('customer and engineer notification persistence, delivery and recovery', async t => {
  ...
  let held;
  await t.test('duplicate form stores one lead and exactly five held jobs', async()=>{
    ...
    assert.equal(held.deliveries.length,3); assert.equal(held.smsDeliveries.length,2);
    assert.deepEqual(held.deliveries.filter(row=>row.purpose==='customer_receipt').map(row=>row.recipient),[enquiry.email]);
    assert.equal(held.deliveries.filter(row=>row.purpose==='team').length,2);
  ...
  await t.test('failure saving an SMS rolls back the lead, event and all email jobs',async()=>{
  ...
  await t.test('parallel workers send each recipient once and keep private form text out of SMS',async()=>{
    ...
    const receipt=mails.find(message=>message.to.address===enquiry.email);
    assert.ok(receipt.text.includes('teşekkür ederiz')&&receipt.html.includes('Talebinizi aldık'));
    assert.equal(receipt.replyTo,process.env.CRM_EMAIL_FROM);
    assert.equal(receipt.headers['Auto-Submitted'],'auto-generated');
    for(const content of [receipt.text,receipt.html]) {
      for(const privateValue of [enquiry.message,enquiry.phone,enquiry.company,'/admin/','onur.durak@','furkan.cansever@']) assert.ok(!content.includes(privateValue));
    }
    assert.ok(mails.filter(message=>message!==receipt).every(message=>message.text.includes(enquiry.message)&&message.replyTo.address===enquiry.email));
  ...
  await t.test('one SMTP recipient failure does not prevent the other recipient or SMS',async()=>{
    ...
    mock.method(nodemailer,'createTransport',()=>({close(){},async sendMail(message){if(message.to.address.startsWith('onur'))throw Object.assign(Error('connection'),{code:'ECONNECTION'});return {accepted:[message.to.address],rejected:[]};}}));
    mock.method(globalThis,'fetch',async()=>smsAccepted(String(200+ ++sms)));
    await processNotifications(item.lead.id);
    const updated=await repo.getLead(item.lead.id);assert.deepEqual(updated.deliveries.map(row=>row.status).sort(),['failed','sent','sent']);assert.equal(sms,2);
  ...
  await t.test('engineer submitting a form still receives distinct customer and team emails',async()=>{
    const key=randomUUID();const input={...enquiry,email:'onur.durak@asirsolar.com'};
    const created=await repo.createWebsiteLead(input,key);await repo.createWebsiteLead(input,key);
    const item=await repo.getLead(created.id);
  ...
  await t.test('untrusted fields are escaped in team HTML and omitted from customer receipts',async()=>{
    const item=await create();const lead={...item.lead,name:'<img src=x onerror=alert(1)>',company:'<script>bad()</script>',message:'<a href="https://evil.invalid">click</a>'};
    const team=renderNotificationEmail('team',lead,'https://www.asirsolar.com');
    assert.ok(team.html.includes('&lt;script&gt;')&&!team.html.includes('<script>'));
    assert.ok(team.html.includes('&lt;a href=&quot;https://evil.invalid&quot;&gt;'));
    const receipt=renderNotificationEmail('customer_receipt',lead,'https://www.asirsolar.com');
    assert.ok(!receipt.html.includes('evil.invalid')&&!receipt.html.includes('/admin/'));
    const sms=renderTeamSms({...lead,name:'Mehmet\nŞahin'},'https://www.asirsolar.com');
    assert.ok(sms.includes('Mehmet Şahin, çatı projesi')&&!sms.includes('\n')&&!sms.includes(lead.message));
  });
  await t.test('public form rejects address lists, groups and header injection',()=>{
    for(const email of ['a,b@example.invalid','a;b@example.invalid','Group:a@example.invalid;','<a@example.invalid>','a@example.invalid\r\nBcc:other@example.invalid']) assert.throws(()=>parsePublicEnquiry({...enquiry,email}),error=>error.status===400);
    assert.equal(parsePublicEnquiry({...enquiry,email:'customer+solar@example.invalid'}).email,'customer+solar@example.invalid');
  });
});
